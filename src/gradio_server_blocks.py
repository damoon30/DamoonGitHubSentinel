...
def operate_project(repo: str, operation: str):
    LOG.info(f"repo={repo}, operate={operation}")
    if operation == "add":
...
with gr.Blocks() as demo:
    ...
    with gr.Tab("view the Github Sentinel"):
        with gr.Row():
            with gr.Column():
                repo = gr.Dropdown(subscription_manager.list_subscriptions(), label="订阅列表", info="已订阅GitHub项目")
                days = gr.Slider(value=2, minimum=1, maximum=7, step=1, label="报告周期",
                                 info="生成项目过去一段时间进展，单位：天")
            with gr.Column():
                mark_result = gr.Markdown()
                download_url = gr.File(label="下载报告")
    ...
    with gr.Tab("manage the Github Sentinel"):
        with gr.Row():
            with gr.Column():
                ...
                operation = gr.Radio(["add", "remove"])
            image_output = gr.Textbox(label="Output")
        image_button = gr.Button("operate")

    view_button.click(export_progress_by_date_range, inputs=[repo, days], outputs=[mark_result, download_url])
    image_button.click(operate_project, inputs=[project_name, operation], outputs=image_output)
